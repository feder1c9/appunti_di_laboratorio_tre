...
Grazie alla sua interfaccia utente semplice e intuitiva, Arduino è stato utilizzato in migliaia di progetti e applicazioni diverse. Il software Arduino è facile da usare per i principianti, ma al contempo sufficientemente flessibile per gli utenti più esperti. Funziona su Mac, Windows e Linux. Insegnanti e studenti lo utilizzano per costruire strumenti scientifici a basso costo, per dimostrare principi di chimica e fisica o per avvicinarsi alla programmazione e alla robotica. Designer e architetti realizzano prototipi interattivi, musicisti e artisti lo usano per installazioni e per sperimentare nuovi strumenti musicali. 
  ...
  \begin{itemize}[leftmargin=1.2cm, itemsep=1pt, topsep=5pt]
    \item \textit{la barra superiore}. Partendo da sinistra troviamo:
    ...
    \item \textit{la barra laterale}. Partendo dall'alto troviamo:
          ...
          \end{itemize}
    \item \textit{l'editor di testo}: la parte più importante dove all'interno viene scritto il vero e proprio codice che ci permetterà di gestire variabili, sensori ed attuatori mediante alcune funzioni.
  \end{itemize}

  Il codice Arduino è sempre diviso in due parti. Queste parti sono rappresentate dalle funzioni \lstinline[style=myArduino]{void setup()} e \lstinline[style=myArduino]{void loop()}. 
  
  Dentro il \lstinline[style=myArduino]{void setup()} vengono inserite tutte le istruzioni che vanno eseguite soltanto una ed una sola volta durante l'esecuzione: queste rappresentano appunto il setup, come ad esempio l'inizializzazione pin di input e output, dello stato dei pin o della comunicazione USB. 

  All'interno del \lstinline[style=myArduino]{void loop()} vanno inserite tutte le istruzioni che verranno ripetute ciclicamente durante l'esecuzione del programma, come ad esempio l'accensione e lo spegnimento periodico di un led, di un controllo di un sensore ecc.
  ...
  \section{Arduino Uno}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.2]{image/foto_arduino/alta_impedenza.png}
