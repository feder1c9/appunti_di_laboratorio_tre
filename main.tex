...
Nella vita di tutti i giorni ognuno di noi compie delle azioni che gli permette di interfacciarsi e dialogare con il mondo esterno: dallo svegliarsi e prepararsi un caffè, fino a guidare l'auto per andare al lavoro.

In tutte queste attività siamo noi i protagonisti: decidiamo quali azioni intraprendere e quali comportamenti adottare in risposta agli stimoli dell'ambiente.

Portiamo alcuni esempi di azioni quotidiane. Consideriamo un automobilista che sta guidando e arriva davanti a un incrocio regolato da un semaforo. Supponiamo che quest'ultimo sia rosso: allora l'automobilista inizierà a frenare fino ad arrestarsi. Questo accade perché l'automobilista, guardando il colore rosso del semaforo, associa a questo colore il significato di arresto/stop, e quindi durante la marcia, inizierà a premere il pedale del freno fino a quando non si fermerà. Dopodiché, passato qualche secondo, il semaforo tornerà a essere verde. In tal caso, l'automobilista assocerà il colore verde al significato di via libera/partenza e quindi inizierà a premere il pedale dell'acceleratore per poter partire.\footnote{Ovviamente dopo aver inserito la marcia.} \\
...
Prima di passare alla descrizione di ogni sottosistema dobbiamo introdurre alcune definizioni.
  ...
  Un \textit{Sottosistema di Acquisizione ad un solo canale} utilizza un solo sensore o trasduttore per ricevere informazioni dal mondo esterno. Nel caso in cui avessimo più di un sensore o trasduttore parleremmo di un \textit{Sottosistema di Acquisizione Multicanale}.
  ...
  \begin{itemize}[leftmargin=1.2cm, itemsep=1pt, topsep=5pt]
    ...
    \item \textbf{\textit{(A)} Amplificatore e \textit{(T)} Traslatore}
          \begin{itemize}
            \item L’\textbf{Amplificatore} è un circuito in grado di amplificare, cioè moltiplicare per una costante l’ampiezza della grandezza elettrica presente al suo ingresso.
            ...
            \item Il \textbf{Traslatore} è un circuito in grado di sommare un valore costante (positivo o negativo) al segnale spostando la forma d’onda verticalmente della quantità desiderata. \textbf{\textit{Viene in genere utilizzato per azzerare l’offset}}
  ...
  \section{Sistema di acquisizione PASCO}

  Il sistema PASCO è un apparato modulare commercializzato per applicazioni didattiche a partire dai primi anni 2000. Risulta necessario rispetto alle misurazioni tradizionali quando si devono analizzare grandezze fisiche che variano velocemente nel tempo, quando occorre un campionamento frequente o un numero elevato di misurazioni, e per permettere il salvataggio dei dati e l'analisi "on-line". Il sistema si compone di un'interfaccia, una serie di sensori (analogici e digitali), un software di acquisizione e una libreria di esperimenti.

  \begin{figure}[H]
    \centering
    \includegraphics[scale=0.3]{image/foto_sistemi_di_controllo/pasco_generale.png}
    \caption{Schema di acquisizione PASCO}\label{fig:pasco_generale}
  \end{figure}

  Le interfacce fungono da ponte tra i sensori e il computer:

  \begin{itemize}
      \item \textbf{Science Workshop 750}: Modello di fascia alta progettato per acquisizioni ad alta velocità e precisione. È dotata di 4 canali digitali e 3 canali analogici utilizzabili simultaneamente. Sfrutta una porta SCSI per raggiungere una frequenza di campionamento di 250 kHz, larghezza di banda che permette al sistema di operare come un oscilloscopio in tempo reale. Include un generatore di funzioni integrato e, per letture a bassa frequenza ($<$ 100 Hz), applica un \textit{over-sampling} 8x per minimizzare il rumore del segnale.
      \item \textbf{Science Workshop 500}: Interfaccia versatile concepita per la portabilità e l'uso sul campo. Dispone di 2 canali digitali e 3 canali analogici. 
  \end{itemize}

  \paragraph{La tecnologia SCSI}

  Per comprendere le prestazioni dell'interfaccia Science Workshop 750, è fondamentale analizzare lo standard di connessione impiegato: la tecnologia \textbf{SCSI} (\textit{Small Computer System Interface}). Introdotta tra gli anni '80 e '90, la SCSI è una famiglia di standard per il trasferimento dati ad alta velocità progettata per collegare computer e periferiche in modo efficiente.

  Sebbene l'avvento dello standard USB abbia semplificato il collegamento delle periferiche (\textit{Plug-and-Play}), la porta SCSI è stata per anni il punto di riferimento per l'acquisizione dati ad alte prestazioni in ambito scientifico, garantendo una stabilità di campionamento che le connessioni consumer del tempo non potevano offrire.

  \section{Sensori e Trasduttori}
  Il cuore del sistema PASCO è costituito dai sensori, ovvero trasduttori capaci di trasformare una grandezza fisica (posizione, forza, pressione, ecc.) in un segnale elettrico analogico o digitale.

    \subsection{Sensori Analogici}
    I sensori analogici producono un segnale in tensione o corrente che varia con continuità in relazione alla variazione della grandezza fisica esaminata. Tra i più utilizzati in laboratorio figurano:
    \begin{itemize}
        \item \textbf{Sensore di Forza}: Utilizzato per studiare l'interazione tra corpi o la tensione in sistemi statici e dinamici.
        \item \textbf{Sensore di Campo Magnetico}: Permette di mappare l'intensità e la direzione del campo generato da magneti o correnti.
        \item \textbf{Sensore di pH e Umidità}: Impiegati per misurazioni ambientali o elettrochimiche.
        \item \textbf{Accelerometro}: Fondamentale per lo studio dei moti accelerati e delle forze d'inerzia.
    \end{itemize}
    ...
    \begin{figure}[H]
      ...
      \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/foto_sistemi_di_controllo/sensori_1.png}
      \end{minipage}\hfill
      \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/foto_sistemi_di_controllo/sensori_2.png}
      \end{minipage}
      \caption{Sensori analogici della PASCO}\label{fig:sensori_analogici_pasco}
    \end{figure}

    \subsection{Sensori Digitali}
    Questi sensori restituiscono valori discreti (tipicamente stati logici acceso/spento) o treni di impulsi.
    \begin{itemize}
        \item \textbf{Fototraguardo (Photogate)}: Composto da un emettitore e un ricevitore a infrarossi; rileva il tempo di oscuramento del fascio da parte di un oggetto in transito, permettendo di calcolare velocità medie e istantanee con alta precisione.
        \item \textbf{Sensore di Posizione (Sonar)}: Funziona emettendo un treno di impulsi a ultrasuoni (frequenza tipica 120 Hz) e ricevendo l'eco riflesso dall'oggetto. Misurando il tempo di volo dell'impulso, il software calcola la distanza dell'oggetto in tempo reale, permettendo di ricostruire la legge oraria del moto.
        \item \textbf{Sensore di Moto Rotatorio}: Codifica la rotazione di una puleggia in impulsi digitali, convertendoli in spostamento angolare.
    \end{itemize}

    \begin{figure}[H]
      \centering
      \includegraphics[height=4cm]{image/foto_sistemi_di_controllo/sensore_fototraguardo.png}
      \hfill
      \includegraphics[height=4cm]{image/foto_sistemi_di_controllo/sensore_posizione.png}
      \hfill
      \includegraphics[height=4cm]{image/foto_sistemi_di_controllo/sensore_moto_rotatorio.png}
      \caption{Sensori digitali della PASCO}\label{fig:sensori_digitali_pasco}
    \end{figure}

  \section{Software di Acquisizione ed Elaborazione}
  Il software funge da interfaccia utente per la configurazione dell'hardware e l'analisi dei dati raccolti.

  \begin{figure}[H]
      \centering
      \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=1.2\textwidth]{image/foto_sistemi_di_controllo/interfaccia_pasco.png}
      \end{minipage}\hfill
      \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/foto_sistemi_di_controllo/data_studio_pasco.png}
      \end{minipage}
      \caption{Science Workshop e Data Studio}\label{fig:software_pasco}
    \end{figure}

  \begin{itemize}
      \item \textbf{Science Workshop}: Software storico che permette la gestione dei canali, la scelta della frequenza di campionamento e la visualizzazione tramite grafici, tabelle o strumenti a lancetta. È particolarmente utile per l'analisi spettrale tramite la Trasformata di Fourier (FFT).
      \item \textbf{Data Studio}: Evoluzione moderna che offre una gestione più intuitiva dei sensori USB e strumenti di calcolo avanzati direttamente sui dati acquisiti (es. derivazione numerica per ottenere l'accelerazione dalla posizione).
  \end{itemize}

  \section{Esperienze di Laboratorio}
  L'uso dei sensori permette di automatizzare la raccolta dati in esperimenti classici, riducendo l'errore umano e aumentando la densità di campionamento.

  \begin{enumerate}
      \item \textbf{Cinematica sul Piano Inclinato}: Attraverso il sensore di posizione si studia il moto uniformemente accelerato, ricavando direttamente le curve $x(t)$ e $a(t)$.
      \item \textbf{Velocità del Suono}: Misurando il ritardo temporale tra l'emissione di un suono e il ritorno dell'eco (riflesso ad esempio da una pallina o dal fondo di un tubo), si calcola la velocità di propagazione dell'onda sonora in aria.
      \item \textbf{Interferenza Sonora (Battimenti)}: Utilizzando due diapason a frequenze leggermente diverse e un sensore di suono, è possibile visualizzare l'inviluppo dell'onda risultante e misurare la frequenza del battimento.
      \item \textbf{Dinamica degli Urti}: Studio della conservazione della quantità di moto e dell'energia cinetica durante urti elastici o anelastici tra carrelli o palline di diversa natura.
      \item \textbf{Moto del Pendolo}: Analisi delle oscillazioni smorzate e verifica della dipendenza del periodo dall'ampiezza massima per grandi angoli (regime non lineare).
  \end{enumerate}

  \begin{figure}[H]
      \centering
      \includegraphics[scale=0.5]{image/foto_sistemi_di_controllo/velocità_suono_aria.png}
      \caption{Velocità suono in aria}\label{fig:velocita_suono_aria}
    \end{figure}
...
L'elettronica è da sempre un ambito di forte interesse, tanto che per decenni è stata un hobby popolarissimo diffuso attraverso numerose riviste specializzate, sia americane che italiane, le quali guidavano gli appassionati nella realizzazione pratica di circuiti. Nel corso degli anni, le tecnologie di base hanno subìto un'evoluzione profonda: si è passati dall'impiego dei tubi a vuoto (negli anni '30 e '40), ai transistor (negli anni '60), per arrivare poi alla grande rivoluzione dei circuiti integrati logici e analogici negli anni '70. 

Storicamente, l'interconnessione di questi circuiti integrati permetteva la progettazione di sistemi specializzati e la costruzione di progetti anche molto complessi, come i ricevitori radio. Esisteva tuttavia un grosso ostacolo: le funzioni che un circuito poteva eseguire erano determinate in modo fisso e permanente dalle connessioni fisiche (hardware) tra i componenti, e non potevano in alcun modo essere riprogrammate o modificate via software. 

La vera svolta concettuale verso l'elaborazione programmabile si è avuta a cavallo tra gli anni '60 e '70 con la nascita dei microprocessori, come l'Intel 4004 rilasciato nel 1971. Questi dispositivi permisero a un circuito di modificare le proprie funzioni a seconda del software eseguito e costituirono il cuore dei primissimi personal computer commercializzati negli anni '70 e '80, tra cui l'Altair 8800, l'Apple II e il Commodore VIC 20.

Se oggi abbiamo a disposizione personal computer estremamente potenti, farli interagire con il mondo esterno (ad esempio per controllare motori o leggere grandezze ambientali tramite sensori) risulta spesso molto complesso per un utente non specializzato. Di fatto, l'interazione dei computer moderni con l'hardware esterno è limitata quasi esclusivamente alla porta USB. 

Arduino nasce esattamente come soluzione a questo problema: colmare il divario tra i dispositivi di calcolo e il mondo reale. Fornisce un sistema accessibile per interfacciare in modo semplice un microprocessore con l'ambiente esterno, unendo la flessibilità della programmazione software all'elettronica hardware.

Di base, Arduino è un sistema di sviluppo hardware e software che permette la creazione di sistemi di controllo utilizzando delle schede elettroniche dotate di microcontrollore su circuito stampato.

\begin{figure}[H]
  \centering
  \includegraphics[scale=0.2]{image/loghi/png-transparent-arduino-hd-logo.png} \quad \includegraphics[scale=0.4]{image/foto_arduino/tipi_di_schede.png}
...
Grazie alla sua interfaccia utente semplice e intuitiva, Arduino è stato utilizzato in migliaia di progetti e applicazioni diverse. Il software Arduino è facile da usare per i principianti, ma al contempo sufficientemente flessibile per gli utenti più esperti. Funziona su Mac, Windows e Linux. Insegnanti e studenti lo utilizzano per costruire strumenti scientifici a basso costo, per dimostrare principi di chimica e fisica o per avvicinarsi alla programmazione e alla robotica. Designer e architetti realizzano prototipi interattivi, musicisti e artisti lo usano per installazioni e per sperimentare nuovi strumenti musicali.
  ...
  \end{itemize}
    ...
    Prima di analizzare la struttura, è bene ricordare alcune regole sintattiche e concetti base:
    \begin{itemize}[leftmargin=1.2cm, itemsep=1pt, topsep=5pt]
      \item Ogni istruzione deve terminare con il punto e virgola (\texttt{;}).
      \item Il codice è \textit{case sensitive} (distingue maiuscole da minuscole).
      \item I commenti si inseriscono con \texttt{//} (singola riga) o racchiusi tra \texttt{/*} e \texttt{*/} (più righe).
    \end{itemize}

    Le variabili devono essere dichiarate specificandone il tipo prima del loro utilizzo. I principali tipi di dato (\textit{datatypes}) sono:
    \begin{itemize}[leftmargin=1.2cm, itemsep=1pt, topsep=5pt]
      \item \texttt{byte}: 8 bit (valori da 0 a 255).
      \item \texttt{int}: 16 bit con segno (valori interi da -32.768 a 32.767).
      \item \texttt{long}: 32 bit con segno, per numeri interi molto grandi.
      \item \texttt{float}: 32 bit per numeri con la virgola (decimali).
      \item \texttt{boolean}: assume solo i valori \texttt{true} (vero) o \texttt{false} (falso).
      \item \texttt{String}: per memorizzare sequenze di caratteri e testo.
    \end{itemize}

    Per combinare variabili ed eseguire controlli logici, si utilizzano gli operatori matematici (\texttt{+}, \texttt{-}, \texttt{*}, \texttt{/}), le funzioni matematiche avanzate (\texttt{sqrt}, \texttt{abs}, \texttt{pow}, \texttt{sin}, \texttt{cos}), gli operatori di confronto (\texttt{>}, \texttt{<}, \texttt{==}, \texttt{!=}) e gli operatori logici (\texttt{\&\&} per AND, \texttt{||} per OR, \texttt{!} per NOT). 
    Inoltre, per controllare il flusso logico del programma a seconda delle condizioni, si usano costrutti di controllo specifici come \texttt{if}, \texttt{if...else}, e iterazioni con i cicli \texttt{for} e \texttt{do...while}.
    ...
    \begin{itemize}[leftmargin=1.2cm, itemsep=1pt, topsep=5pt]
      \item \lstinline[style=myArduino]{pinMode(pin, INPUT)} e \lstinline[style=myArduino]{pinMode(pin, OUTPUT)}: servono ad inizializzare un pin di Arduino come ingresso o uscita;
      \item \lstinline[style=myArduino]{digitalRead(pin)}: serve a leggere lo stato alto o basso del pin di ingresso;
      \item \lstinline[style=myArduino]{digitalWrite(pin, HIGH)} e \lstinline[style=myArduino]{digitalWrite(pin, LOW)}: servono a cambiare lo stato del pin di uscita. Lo stato \lstinline[style=myArduino]{HIGH} corrisponde a 5V mentre lo stato \lstinline[style=myArduino]{LOW} a 0V;
      \item \lstinline[style=myArduino]{analogRead(pin)}: serve a leggere i valori dell'ingresso analogico. I valori vanno da 0 a 1023 dove anche qua 0 corrisponde a 0 V e 1023 a 5V. Vedremo dopo perché l'ingresso analogico dà questi valori;
      \item \lstinline[style=myArduino]{analogWrite(pin, valore)}: permette di dare una tensione variabile da 0 a 5V al pin di uscita variando il parametro "valore" che va da 0 a 255. Questo è possibile grazie al PWM che approfondiremo in seguito;
      \item \lstinline[style=myArduino]{delay(tempo)}: mette in pausa per un certo tempo la scheda. Quindi, se ad esempio usiamo \lstinline[style=myArduino]{delay(1000)}, l'Arduino rimarrà in attesa per un tempo pari a 1000ms cioè 1 secondo. Durante questo intervallo la scheda riamane "bloccata", nel senso che non leggerà nessun valore di ingresso e non modificherà lo stato dei pin di output. Il microcontrollore sarà occupato semplicemente a contare il tempo che scorre. 
      \item \lstinline[style=myArduino]{delayMicroseconds(us)}: ha un comportamento analogo a \texttt{delay}, ma blocca il microcontrollore per un intervallo di tempo specificato in microsecondi.
      \item \lstinline[style=myArduino]{millis()}: restituisce il numero di millisecondi trascorsi dall'inizio dell'esecuzione del programma. A differenza di \texttt{delay}, non blocca la scheda e permette di eseguire azioni parallele tenendo traccia del tempo. Essendo gestito da un registro interno a 32 bit, raggiunge la sua capienza massima e torna a zero (\textit{overflow}) dopo circa 49,7 giorni.
    \end{itemize}
  ...
  \section{Arduino Uno}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.11]{image/foto_arduino/cavo_maschio_maschio.jpg} \quad \includegraphics[scale=0.07]{image/foto_arduino/cavo_femmina_femmina.png} \quad \includegraphics[scale=0.14]{image/foto_arduino/cavo_maschio_femmina.jpg}
    ...
    \paragraph{Uscita digitale}

    Supponiamo di voler accendere e spegnere un led utilizzando una board Arduino. Quello che faremo allora è prendere il led, collegarlo ad una resistenza per limitarne la corrente e connettere l'altro terminale del led al pin digitale di Arduino.\footnote{Ricordando che il led possiede una polarità.}

    Per poter funzionare, questo pin dovrà essere impostato all'interno del \lstinline[style=myArduino]{void setup()} come uscita digitale attraverso la funzione \lstinline[style=myArduino]{pinMode(pin, OUTPUT)}. Nel \lstinline[style=myArduino]{void loop()} inseriremo la funzione \lstinline[style=myArduino]{digitalWrite(pin, HIGH)} per accendere il led e \lstinline[style=myArduino]{digitalWrite(pin, LOW)} per spegnarlo. 

    Se, però, mettessimo queste funzioni una dietro l'altra quello che accade è che il led rimarrà sempre acceso, e questo perché la velocità con Arduino esegue il programma è molto veloce e non ha il tempo di spegnere il led. Per questo, tra una funzione e l'altra inseriremo la funzione \lstinline[style=myArduino]{delay(tempo)} impostando un certo valore di tempo in millisecondi. 

    Di seguito è riportato un esempio di utilizzo del pin digitale in output.

    \begin{lstlisting}[style=myArduino, basicstyle=\ttfamily\footnotesize, caption={Esempio lampeggio led}, captionpos=b, label=code:lampeggio_LED]
    #define LED 6
    ...
    Colleghiamo quindi un'estremità (o terminale) del pulsante al pin di Arduino e l'altro al GND, ovvero il terminale posto a potenziale 0 V. Dentro il \lstinline[style=myArduino]{void loop()} ci sarà la parte di programma che accenderà un led quando premiamo il pulsante e che lo spegnerà quando non lo premiamo.

    Sembrerebbe essere molto semplice: quando dal pin di ingresso viene letto uno stato logico basso viene acceso il led, quando viene letto uno stato logico alto viene spento. Ebbene, in questo ragionamento che abbiamo fatto c'è però un problema. Quando premiamo il pulsante tutto funziona correttamente: essendo collegato al GND si porta ad uno stato logico basso e il led viene acceso. Invece, quando non lo premiamo lo stato del pulsante non è determinato, proprio perché non colleghiamo fisicamente il pin a nessun punto del circuito in cui ci sia una tensione elettrica. Il led potrebbe quindi o accendersi o spegnersi o addirittura lampeggiare proprio perché lo stato del pin di ingresso viene influenzato dal rumore esterno.
    ...
    Come si vede in \cref{fig:pull_up_pull_down} queste sono le due configurazioni che ci serviranno per collegare un pulsante o un interruttore ad Arduino. La resistenza di pull-up e di pull-down sono solitamente dell'ordine dei $k\Omega$ per limitare il passaggio di corrente all'interno del circuito\footnote{Evita di bruciare tutto}.

    Nella configurazione di pull-up la resistenza viene collegata tra il nodo di alimentazione e il pulsante. Ricordando che all'interno del pin si ha un'alta impedenza, vediamo cosa accade nel caso in cui il pulsante viene premuto oppure no:
    \begin{itemize}[leftmargin=1.2cm, itemsep=1pt, topsep=5pt]
      \item \textit{Pulsante premuto}: quando premiamo il pulsante, colleghiamo la resistenza al nodo GND. Il pin di Arduino sarà anch'esso collegato al nodo GND e quindi la tensione letta dal pin sarà di 0V, proprio perché si tratta dello stesso nodo circuitale. La corrente passerà tranquillamente dai 5V al GND attraverso la resistenza.
      \item \textit{Pulsante non premuto}: quando il pulsante non è premuto il pin è collegato solo alla resistenza. La corrente passerà attraverso la resistenza di pull-up e il pin ma essendo quest'ultimo in alta impedenza la corrente è molto piccola, quasi nulla, come se il circuito fosse aperto. Il pin corrisponderà al nodo di alimentazione e quindi la tensione letta sarà proprio 5 V.
    \end{itemize}

    Arduino permette di inizializzare i pin digitali con una resistenza di pull-up interna utilizzando la funzione \lstinline[style=myArduino]{pinMode(pin, INPUT_PULLUP)}, così da poter collegare direttamente il pulsante tra pin e GND senza utilizzare resistenze esterne. Qui di seguito riportiamo un esempio:

    \begin{lstlisting}[style=myArduino, basicstyle=\ttfamily\footnotesize, caption={Esempio pulsante}, captionpos=b, label=code:pulsante]
    #define PULSANTE 5
    #define LED      6
    ...
    void loop(){
      valore_puls = digitalRead(PULSANTE);
      if(valore_puls == LOW){
        digitalWrite(LED, HIGH);
      }
      else {
    ...
    Il breve \lstinline[style=myArduino]{delay(100)} serve principalmente a stabilizzare le letture dei sensori, ridurre il carico computazionale del microcontrollore e rendere più leggibili i dati sulla seriale. 



    \paragraph{Ingresso analogico}

    Quando abbiamo un sensore o un trasduttore che produce un segnale di tipo analogico, per poter leggere i valori dobbiamo collegarlo ad un pin analogico. 

    I pin analogici di Arduino sono collegati ad un convertitore analogico digitale (ADC) che, appunto, converte i segnali analogici in segnali digitali. In particolare, il convertitore di Arduino è un ADC a 10 bit, ciò significa che converte segnali che vanno da 0V a 5V in valori da 0 a 1023.\footnote{Per calcolare il numero di combinazioni basta fare $2^{10} = 1024$} La risoluzione di questo ADC si calcola molto semplicemente come $R = \frac{5 V}{2^{10}} = \frac{5 V}{1024} = 4.88mV$, ovvero ogni bit dell'ADC corrisponderà ad un valore di $4.88mV$.

    All'interno del \lstinline[style=myArduino]{void setup()} non sarà necessario inizializzare il pin che vogliamo utilizzare con \lstinline[style=myArduino]{pinMode(pin, INPUT)} perché di default gli ingressi analogici di Arduino sono impostati come pin di ingresso. Dentro il \lstinline[style=myArduino]{void loop()}, per leggere il segnale analogico, bisogna utilizzare la funzione \lstinline[style=myArduino]{analogRead(pin)} e salvare questi valori in una variabile. Di seguito un esempio.

    \begin{lstlisting}[style=myArduino, basicstyle=\ttfamily\footnotesize, caption={Esempio comunicazione seriale}, captionpos=b, label=code:comunicazione_seriale]
    const int analogPin = A0; 

    void setup() {
      // Inizializza la comunicazione seriale a 9600 baud per stampare i dati
      Serial.begin(9600);
    }

    void loop() {
      // Legge il valore della tensione sul pin analogico A0
      ...
      // Pausa di 100 millisecondi per non saturare la comunicazione seriale
      delay(100);
    }
    \end{lstlisting}

    Un'applicazione classica è la misurazione dell'intensità luminosa tramite una \textbf{fotoresistenza}. Poiché questo componente varia la propria resistenza in base alla luce, non può essere collegato direttamente al pin analogico per fornire una tensione. Va invece inserito in un partitore di tensione, mettendolo in serie con una resistenza fissa (es. 10 $k\Omega$). La caduta di tensione misurata nel punto di giunzione tra i due componenti varierà tra 0 V e 5 V proporzionalmente alla luce ricevuta, permettendone la corretta lettura con \lstinline[style=myArduino]{analogRead()}.

    \paragraph{Uscita analogica}

    Riprendiamo l'esempio del led. Prima utilizzavamo il pin digitale per accendere e spegnere il led, cioè sfruttavamo la funzione \lstinline[style=myArduino]{digitalWrite(pin, valore)} per commutare da 0V a 5V il led.  

    Se adesso volessimo modulare la luminosità del led allora dovremmo cambiare la tensione applicata al led. Solitamente nelle board di questo tipo è presente un componente che fa il lavoro opposto dell'ADC ovvero il convertitore digitale analogico (DCA) che converte un valore digitale in un valore analogico.

    Arduino, però, non dispone di un DCA al suo interno. Per poter avere una tensione analogica in uscita viene sfruttata un'altra tecnica: il \textit{PWM}.\\
    Il PWM (Pulse Width Modulation) ovvero modulazione d'ampiezza di impulso, è una tecnica che permette di generare un segnale di onda quadra con un \textit{duty cycle} variabile.
    
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.35]{image/foto_arduino/duty_cycle.png}
        \caption{Duty cycle}\label{fig:duty_cycle}
    \end{figure}

    Il duty cycle è definito come quell'intervallo di tempo in cui si ha lo stato alto rispetto al periodo totale:

    \begin{equation*}
      D  = \frac{t_{on}}{T}
    \end{equation*}
    
    e per come è definito può variare tra 0 e 1. Solitamente si riporta il valore in percentuale quindi tra 0 \% e 100 \%. Variare questo parametro D del segnale permette di ottenere una tensione variabile dipendente dal rapporto tra la durata dell'impulso positivo e di quello totale.

    Introduciamo il concetto di \textit{valore efficace RMS} (Root Mean Square) di una funzione, definito come la media quadratica sul periodo della funzione stessa:

    \begin{equation*}
      V_{RMS} = \sqrt{\frac{1}{T} \int_{0}^{T}v_{pwm}^{2}(t) dt}
    \end{equation*}

    In genere, si applica a segnali di tensione o corrente al fine di calcolare gli effetti sul carico. In altre, parole il valore efficace (di tensione o di una corrente) è quel valore di segnale in continua che produce su una resistenza gli stessi effetti (dissipazione di potenza) di un segnale non continuo.

    \begin{equation*}
      \langle P \rangle = V_{RMS} I_{RMS}  
    \end{equation*}

    ovvero, il valore efficace è equivalente al valore della corrente continua, in grado di produrre la stessa dissipazione di potenza in un carico resistivo. Nel caso di onda quadra con ampiezza 5V e duty cycle D, si ha:

    \begin{equation*}
      V_{RMS} = \sqrt{\frac{1}{T}\biggl[ \int_{0}^{t_{on}} 5^2 dt + \int_{t_{on}}^{T} 0^2 dt \biggr]} = \sqrt{\frac{1}{T} 5^2 \biggr[ \int_{0}^{t_{on}} 1 dt \biggl]} =\sqrt{\frac{t_{on}}{T} 5^2} =\sqrt{5^2 D}
    \end{equation*}
    ...
    da questa ultima espressione è evidente che il valore efficace sarà dipendente da D, ottenendo valori compresi tra 0 e $V_{max}$. 

    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.25]{image/foto_arduino/variazione_duty_cycle.jpg}
        \caption{Variazione del Duty cycle}\label{fig:variazione_duty_cycle}
    \end{figure}

    In questo modo si potranno pilotare diversi dispositivi sfruttando questa modulazione di segnale. Ad esempio, applicazioni tipiche del PWM sono il controllo della luminosità dei LED, il controllo di motori, la gestione delle ventole del pc, ecc.

    All'interno del codice, per poter utilizzare il PWM, si utilizza la funzione \lstinline[style=myArduino]{analogWrite(pin, valore)} dove valore indica il valore da 0 a 255 del duty cycle, proporzionato in una scala percentuale. Quindi un valore di 255 rappresenta un duty cycle al 100\%, un valore di 126 indica un duty cycle al 50\% e così via. 

    \begin{lstlisting}[style=myArduino, basicstyle=\ttfamily\footnotesize, caption={Esempio uscita analogica}, captionpos=b, label=code:pwm]
    #define LED 9 

    void setup() {
      // A differenza delle letture analogiche, i pin usati per il PWM 
      // devono essere esplicitamente dichiarati come OUTPUT
      pinMode(LED, OUTPUT);
    }

    void loop() {
      // Ciclo per aumentare gradualmente il segnale da 0 a 255
      for (int luminosita = 0; luminosita <= 255; luminosita++) {
        analogWrite(LED, luminosita);
        // Una piccola pausa per rendere l'effetto visibile all'occhio umano
        delay(10); 
      }

      // Ciclo per diminuire gradualmente il segnale da 255 a 0
      for (int luminosita = 255; luminosita >= 0; luminosita--) {
        analogWrite(pwmPin, luminosita);
        delay(10);
      }
    }
    \end{lstlisting}

    \paragraph{Comunicazione seriale}

    La comunicazione seriale è il sistema con cui Arduino scambia dati con il computer (via cavo USB) o con altri componenti (usando i pin fisici RX per la ricezione e TX per la trasmissione). Viene usata principalmente per due scopi: inviare dati al PC per visualizzarli sul Monitor Seriale (fase di test e debugging) o ricevere comandi dal PC. 

    Quando dobbiamo inizializzare la comunicazione seriale è necessario inserire un numero che rappresenta ila velocità di comunicazione. Questo valore è chiamato Baud (o Baud Rate)\footnote{E non i Baudi del fantasanremo} ed indica,appunto, la velocità di trasmissione dei dati, misurata in bit al secondo (bps). 

    La regola che si segue è che Arduino e il dispositivo con cui comunica (come il Monitor Seriale dell' IDE) devono essere impostati esattamente allo stesso baud rate. Se le velocità non coincidono, il ricevitore interpreterà male i segnali elettrici e sullo schermo appariranno solo caratteri incomprensibili. Lo standard storico è di 9600 baud, sufficiente per quasi tutti i progetti di base. Valori più alti come 115200 vengono usati quando si devono trasferire grandi quantità di dati molto rapidamente (ad esempio, sensori di movimento ad alta frequenza o moduli Wi-Fi).

    All'interno del codice, nel \lstinline[style=myArduino]{void setup()} andrà inserito quindi \lstinline[style=myArduino]{Serial.begin(9600)} e nel \lstinline[style=myArduino]{void loop()} andrà inserito:

    \begin{itemize}[leftmargin=1.2cm, itemsep=1pt, topsep=5pt]
      \item \lstinline[style=myArduino]{Serial.print(valore)}: Invia il dato al Monitor Seriale e lascia il cursore alla fine del dato appena stampato. Qualsiasi istruzione di stampa successiva continuerà sulla stessa riga, attaccata a destra.
      \item \lstinline[style=myArduino]{Serial.println(valore)}: Il suffisso "ln" sta per line. Invia il dato e poi aggiunge i comandi invisibili di "ritorno a capo". L'istruzione di stampa successiva ripartirà dalla riga sottostante.
    \end{itemize}

  \section{Applicazioni Pratiche e Sensori Avanzati}
  
  Oltre al semplice controllo di LED e pulsanti, le potenzialità di Arduino si estendono verso l'interfacciamento con il mondo fisico attraverso moduli più sofisticati.

  \paragraph{Generazione di suoni}
  Per la generazione di segnali acustici, è possibile collegare un \textit{buzzer passivo} a un pin digitale. A differenza di un buzzer attivo (che suona non appena riceve 5 V), un buzzer passivo necessita di un'onda quadra modulata per vibrare. Utilizzando la funzione \lstinline[style=myArduino]{tone(pin, frequenza)} si genera l'onda desiderata, mentre con \lstinline[style=myArduino]{noTone(pin)} se ne interrompe l'esecuzione. Mappando all'interno di un \textit{array} le frequenze corrispondenti alle note musicali (es. Do=131 Hz, Re=147 Hz) e utilizzando un ciclo \texttt{for}, è possibile programmare sequenze ritmiche e riprodurre melodie intere.

  \paragraph{Acquisizione di segnali impulsivi (Contatore Geiger)}
  Le letture sui pin digitali avvengono in modo estremamente rapido, impiegando mediamente solo circa 10 $\mu s$ per campionamento. Questa velocità consente di interfacciarsi con strumenti di precisione come i rivelatori di radiazioni. Ad esempio, collegando l'uscita TTL di un \textbf{contatore Geiger} a un pin digitale, ogni rilevazione di particella si tradurrà in un brevissimo impulso da 5 V. Utilizzando la funzione \lstinline[style=myArduino]{millis()} all'interno di un blocco \lstinline[style=myArduino]{if} che riconosce il passaggio di stato (da LOW a HIGH), il sistema non solo conterà il numero di eventi cumulativi, ma potrà calcolare istante per istante la distanza temporale ($\Delta t$) intercorsa tra un decadimento e l'altro.

  \begin{figure}[H]
        \centering
        \includegraphics[scale=0.5]{image/foto_arduino/buzzer.jpg} \ \includegraphics[scale=0.15]{image/foto_arduino/geiger.jpg}
        \caption{Buzzer e Geiger}\label{fig:buzzer_geiger}
  \end{figure}

  \section{Librerie ed Espansioni Hardware}

  Quando un progetto cresce in complessità, non è sufficiente gestire manualmente le oscillazioni dei singoli pin, ma si ricorre a librerie software ed espansioni fisiche.

  \paragraph{Le Librerie e i Display}
  Una libreria è una collezione di istruzioni e funzioni pre-compilate progettata per semplificare drasticamente la comunicazione con specifici moduli elettronici. Per includere una libreria si utilizza la direttiva \lstinline[style=myArduino]{#include <nome_libreria.h>} all'inizio dello \textit{sketch}.
  Questa procedura è obbligatoria, ad esempio, per pilotare output visivi complessi:
  \begin{itemize}[leftmargin=1.2cm, itemsep=1pt, topsep=5pt]
      \item \textbf{Display LCD}: come il classico modulo a 2 righe e 16 caratteri alfanumerici.
      \item \textbf{Matrici LED}: moduli 8x8 o 32x16, utili per visualizzare testo scorrevole o animazioni bidimensionali.
  \end{itemize}

  \begin{figure}[H]
        \centering
        \includegraphics[scale=0.27]{image/foto_arduino/display_lcd.jpg} \ \includegraphics[scale=0.3]{image/foto_arduino/display_matrice.jpg}
        \caption{Display LCD e a Matrice di LED}\label{fig:displays}
  \end{figure}

  \paragraph{Gli Shield}
  Dal punto di vista hardware, Arduino può essere potenziato impiegando i cosiddetti \textit{Shield}. Si tratta di circuiti stampati modulari progettati per essere impilati direttamente sopra la scheda principale, agganciandosi alla medesima piedinatura.
  Il mercato offre shield per qualsiasi applicazione: moduli Wi-Fi ed Ethernet per connettere la scheda a internet, shield per il controllo di potenza dei motori (\textit{Motor driver}), moduli GPS e lettori per schede SD. L'assemblaggio combinato di sensori e shield di connettività rappresenta la base per progetti di domotica (la casa intelligente) e robotica automatizzata.

  \begin{figure}[H]
        \centering
        \includegraphics[scale=0.28]{image/foto_arduino/shield_wifi.jpg} \ \includegraphics[scale=0.5]{image/foto_arduino/shield_sd.jpg}
        \caption{Shield wifi e SD}\label{fig:shield_wifi_sd}
  \end{figure}

  \section{Simulazione di Circuiti Elettronici}

  L'analisi e la progettazione di un circuito elettronico reale sono attività che, storicamente, potevano risultare lente e costose, soprattutto a causa del rischio intrinseco di danneggiare i componenti durante le fasi di collaudo su \textit{breadboard}. Oggi, con la progressiva miniaturizzazione dei transistor (come descritto empiricamente dalla \textit{Legge di Moore}), i progettisti evitano la prototipazione fisica immediata preferendo affidarsi alla \textbf{simulazione circuitale}.

  I simulatori odierni si basano su complessi modelli numerici (es. SPICE) capaci di emulare con alta fedeltà il comportamento fisico dei componenti. Questa pratica garantisce numerosi vantaggi:
  \begin{itemize}[leftmargin=1.2cm, itemsep=1pt, topsep=5pt]
      \item Permette l'analisi di circuiti senza disporre fisicamente dell'hardware.
      \item Previene rotture e cortocircuiti accidentali.
      \item Abbassa radicalmente i costi di sviluppo e il tempo di rilascio sul mercato (\textit{time-to-market}).
  \end{itemize}
  Sebbene una simulazione non possa sostituire al 100\% i test di laboratorio (poiché è impossibile modellare ogni singola variabile o interferenza del mondo reale), rappresenta uno strumento fondamentale in ingegneria.

  Nel contesto di Arduino, esistono numerosi ambienti di sviluppo, molti dei quali basati su infrastruttura Cloud e totalmente gratuiti:
  \begin{itemize}[leftmargin=1.2cm, itemsep=1pt, topsep=5pt]
      \item \textbf{Tinkercad}: È una delle piattaforme educative più complete (prodotta da Autodesk). Offre un'interfaccia interattiva che permette di assemblare la propria breadboard con LED, resistenze e sensori (come fotoresistenze o buzzer), scrivere il codice e avviare la simulazione, osservando l'esecuzione del programma e i messaggi del Serial Monitor in tempo reale.
      \begin{figure}[H]
        \centering
        \includegraphics[scale=0.3]{image/foto_arduino/tinkercad.png}
        \caption{Tinkercad}\label{fig:tinkercad}
      \end{figure}

      \item \textbf{Fritzing}: Ideale per disegnare in maniera chiara la topologia dei collegamenti prima di cablarli realmente, per poi esportarli come schemi elettrici o progetti per circuiti stampati (PCB).
      \begin{figure}[H]
        \centering
        \includegraphics[scale=0.7]{image/foto_arduino/fritzing.jpg}
        \caption{Fritzing}\label{fig:fritzing}
      \end{figure}

      \item \textbf{LTSpice e PSpice}: Software di simulazione professionale impiegati quando è richiesta l'analisi matematica rigorosa delle correnti e delle tensioni ai nodi del circuito. 
      \begin{figure}[H]
        \centering
        \includegraphics[scale=0.7]{image/foto_arduino/ltspice.jpg}
        \caption{LTSpice}\label{fig:ltspice}
      \end{figure}
  \end{itemize}
...
neutroni.
\begin{center}
    \includegraphics[width=6.5cm]{nuclei_stabili.png}
\end{center}
\begin{itemize}
    \item $1<Z<20$: i nuclei sono stabili quando il numero di protoni è uguale al numero di neutroni;
    \item $Z>20$: i nuclei sono stabili quando il numero di neutroni è maggiore al numero di protoni, questo perché il numero maggiore di neutroni serve a ridurre l'effetto repulsivo coulumbiano tra i protoni, se no si avrebbe la disintegrazione del nucleo dovuto all'allontanamento dei protoni.
...
\begin{itemize}
    \item \'E proporzionale a $z^2$, ovvero alla carica della particella incidente (le particelle alfa perdono 4 volte più carica dei protoni), serve a identificare la particella;
...
\begin{itemize}
    ...
    $$e^-+X\rightarrow X^-+h\nu$$
\end{itemize}
Per essere rivelati, elettroni e ioni positivi devono sopravvivere per un tempo almeno pari al tempo di raccolta degli elettroni, in altre parole, affinché le particelle vengano rivelate il tempo di risposta dello strumento deve essere minore del tempo di ricombinazione. Per tale ragione spesso la scelta del gas verte su quelli costituiti da molecole non molto elettronegative.\\
