...
\definecolor{giallo}{rgb}{245, 208, 51}
\usepackage{multirow}
\usepackage{amsthm}
\usepackage{amssymb}
\usepackage{amsmath}
...
\usepackage{yhmath}
\usepackage{graphicx}
\graphicspath{ {image/foto/} {image/loghi/} }
...
\begin{itemize}[leftmargin=1.2cm, itemsep=1pt, topsep=5pt]
  ...
  \item Elaborare queste informazioni mediante\footnote{Come vedremo più avanti stiamo parlando di circuiti elettrici ed elettronici che sono molto utili per gestire e manipolare le informazioni. Negli esempi precedenti il sistema di elaborazione è il cervello umano.}:
        \begin{itemize}[itemsep=1pt, topsep=2pt]
          \item Circuiti logici -- Sequenziali (Flip-Flop, Shift Register, Contatori)\footnote{Sono dei componenti base della logica sequenziale.} o Combinatori (And, Or, Not); % chktex 8
          \item Circuiti analogici -- Amplificatori Operazionali (Invertente, Non invertente, Sommatori, Differenziali, Esponenziali, Logaritmici)\footnote{Si tratta di amplificatori operazionali in configurazioni differenti. Ognuno di questi ha un nome che ci permette di capire come verrà manipolato il segnale analogico}. % chktex 8
        \end{itemize}
...
Prima di passare alla descrizione di ogni sottosistema dobbiamo introdurre alcune definizioni.
  ...
  \begin{itemize}[leftmargin=1.2cm, itemsep=1pt, topsep=5pt]
    ...
    \item \textbf{\textit{(A)} Amplificatore e \textit{(T)} Traslatore}\\ % chktex 8
...
Grazie alla sua interfaccia utente semplice e intuitiva, Arduino è stato utilizzato in migliaia di progetti e applicazioni diverse. Il software Arduino è facile da usare per i principianti, ma al contempo sufficientemente flessibile per gli utenti più esperti. Funziona su Mac, Windows e Linux. Insegnanti e studenti lo utilizzano per costruire strumenti scientifici a basso costo, per dimostrare principi di chimica e fisica o per avvicinarsi alla programmazione e alla robotica. Designer e architetti realizzano prototipi interattivi, musicisti e artisti lo usano per installazioni e per sperimentare nuovi strumenti musicali. 
  ...
  \section{Il linguaggio di programmazione}
    ...
    Arduino UNO è una scheda a microcontrollore basata sull'ATmega328P. Dispone di diversi ingressi:
    ...
    \begin{figure}[H]
    \centering
    \includegraphics[scale=0.1]{image/foto_arduino/arduino_uno_esempio.jpg}
    \caption{Esempio di utilizzo di Arduino Uno}\label{fig:arduino_uno_esempio}
    \end{figure}

    Dopo aver visto le caratteristiche tecniche dell'Arduino Uno, andiamo a vedere che cosa sono e a cosa servono.

    \subsection{I Pin di Arduino}

    In generale, i pin sono quei componenti di un circuito che permettono il collegamento con altri dispositivi; in questo caso i pin sono dei fori che si trovano direttamente sulla scheda con cui possiamo collegare altre boards, sensori ed atturatori mediante dei cavetti.
    ...
    %Foto alta impedenza ingresso di arduino
    ...
    %Foto resistenza PULLUP e PULLDOWN
...
\end{center}
\section{\texorpdfstring{Esempio: il decadimento doppio $^{90}Sr/^{90}Y$}{Esempio: il decadimento doppio 90Sr/90Y}}
...
\section{\texorpdfstring{Decadimento $\alpha$}{Decadimento alpha}}
...
\section{\texorpdfstring{Decadimento $\gamma$}{Decadimento gamma}}
...
Tali rivelatori a gas hanno due regimi di funzionamento: Streamer o valanga.
\begin{figure}[h!]
    \centering
...
Questi rivelatori vengono preparati facendo diffondere impurità di tipo n, come ad esempio il fosforo, in cristalli di silicio di tipo p ad alta resistività.\\
...
\begin{center}
    \includegraphics[width=15cm]{pompa7.png}
\end{center}
...
\begin{center}
    \includegraphics[width=15cm]{multimetro1.png}
\end{center}
